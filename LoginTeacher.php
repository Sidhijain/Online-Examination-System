    ...
    <div class="main">
        <form action="#" method="POST">
            ...
            <div class="input_field">
                <label>User ID :</label>
                <input type="text" class="input" name="Tid" placeholder="Enter Your User ID" required/>
            </div>  
            <br>
            <div class="input_field">
...
{ 
    ...
    $query="SELECT * from teacher WHERE Tid= '$id' AND Pass='$ps'";
    $data=mysqli_query($conn, $query);  // if conn done then my query should be inserted
    $res=mysqli_num_rows($data);

              if($res===1)
                {
                    $idt_pass=mysqli_fetch_assoc($data);
                    
                    // $db_pass=$id_pass['Pass'];

                    $_SESSION['Tid']=$idt_pass['Tid'];
                    $_SESSION['Tname']=$idt_pass['Tname'];
                    $_SESSION['Dept']=$idt_pass['Dept'];
                    $_SESSION['Phno']=$idt_pass['Phno'];
                    $_SESSION['Email']=$idt_pass['Email'];

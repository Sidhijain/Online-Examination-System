    ...
    <div class="main">
        <form action="#" method="POST">
            ...
            <div class="input_field">
                <label>User ID :</label>
                <input type="text" class="input" name="Sid" placeholder="Enter Your User ID" required/>
            </div>  
            <br>
            <div class="input_field">
            ...
            <div class="signup">
                If you are new:
                    <button class="b2" > <a href="SignUpStudent.php">Sign Up</a></button>
            </div>
            </form>   
       </div>
        </form>
...
{ 
    ...
    $query="SELECT * from student WHERE Sid= '$id' AND Pass='$ps'";
    $data=mysqli_query($conn, $query);  // if conn done then my query should be inserted
    $res=mysqli_num_rows($data);
              if($res===1)
                {
                    $ids_pass=mysqli_fetch_assoc($data);
                    
                    // $db_pass=$id_pass['Pass'];

                    $_SESSION['Sid']=$ids_pass['Sid'];
                    $_SESSION['Eno']=$ids_pass['Eno'];
                    $_SESSION['Rollno']=$ids_pass['Rollno'];
                    $_SESSION['Name']=$ids_pass['Name'];
                    $_SESSION['Email']=$ids_pass['Email'];
                    $_SESSION['Contactno']=$ids_pass['Contactno'];
                    $_SESSION['Altcontactno']=$ids_pass['Altcontactno'];
